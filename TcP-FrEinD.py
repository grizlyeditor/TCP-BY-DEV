...
# Global variables for current bot session
current_key = None
current_iv = None
current_region = None
    ...
    while True:
        try:
            ...
            if action == 'join_team':
                ...
                if team_code and online_writer:
                    # Join the team
                ...
                if emote_id and target_uids and online_writer:
...
def handle_emote():
    ...
    try:
        ...
        account = ACCOUNTS.get(key, DEFAULT_ACCOUNT)
        ...
        # Add command to queue using thread-safe method
        add_api_command({
            'action': 'join_team',
            'team_code': team_code,
            'region': server
        ...
        if uids_list:
            ...
            def delayed_emote():
                ...
                # Auto leave after emote is sent
                time.sleep(2)
                add_api_command({
                    'action': 'leave_team'
                })
            ...
            # If no UIDs specified, just join and leave
            def delayed_leave():
                time.sleep(5)
                add_api_command({
                    'action': 'leave_team'
        ...
        return jsonify({
            'status': 'success', 
            'message': f'Command queued: Join {team_code} and send emote {emote_id}',
            'targets': uids_list,
            'account_used': key if key in ACCOUNTS else 'DEFAULT'
        }), 200
